...
## EMR Migration: EMR 7.12.0 / Spark 3 (CDP-118269)

An active migration is in progress to upgrade pipelines from older EMR releases to **EMR 7.12.0 / Spark 3**. The following pipelines have already been migrated (merged to main):
...
When working on pipelines, check `pipelines/<name>/config/emr.json` to confirm the EMR release version. When migrating a new pipeline, update the EMR release label to `emr-7.12.0` and verify Spark 3 compatibility (particularly UDFs and any deprecated Spark 2 APIs).

Integration test Terraform configs (`terraform/pipelines/<name>/integration/`) have also been added for several pipelines as part of this migration effort.

**Removed pipeline:** `fusion-behavior-preprocess` was removed from the repo (PR #734). Do not add it back.
...
- **EMR 7.12.0 migration in progress:** Multiple pipelines are being migrated to EMR 7.12.0 / Spark 3 (CDP-118269). Check `emr.json` before assuming any pipeline is still on the old EMR version.
