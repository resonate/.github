...
- **Decommissioned pipelines:** `fusion-behavior-preprocess` (#734) and `cookiejar-sample-export` (#748) have been decommissioned — their infra (terraform dirs) has been removed but the pipeline code in `pipelines/` is kept for historical reference with `DEPRECATED.md` markers. Do not redeploy these.
- **CI false positive on terraform dir deletion:** When a PR deletes a `terraform/pipelines/<name>/` directory, the `step_function.yml` workflow fires for the deleted path, correctly skips Terragrunt, but a downstream timestamp step without `dir_exists` guard causes a spurious bash failure. This is a known upstream `workflow-templates` issue — the CI failure is not caused by the PR code itself.
- **EMR 7.12.0 / Spark 3 migration (CDP-118269):** The following pipelines were migrated from EMR 6.x / Spark 2.x to EMR 7.12.0 / Spark 3 in 2026-05/06: `maid-onboarder`, `marketops-overlap`, `damlam-preparation`, `idsync-overlap`, `topic-aggregation`, `behavior-stitch`. Check `pipelines/<name>/config/emr.json` for the current EMR release label.
- **Shuffle partition tuning:** `behavior-stitch` uses increased `spark.sql.shuffle.partitions` to accommodate data growth (see PR #681). If adding new pipelines that process large datasets, tune this per pipeline rather than relying on defaults.
- **Geo-location pipeline zip-district mappings (CDP-118946):** State-house, state-senate, and NH district mappings have been rewritten to the L2 namespace. The geo-location pipeline threads zip-district data through with `district_source` provenance and IP-inferred fields. When modifying geo-location configs, be aware of both the L2 namespace rewrites and the ToBitmap gating logic.
