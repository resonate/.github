...
2. **Stitch** - Scala/Spark job that joins evaluated expressions with vendor identity tables (cookies, HEMs, device IDs, person RIDs)
...
**Important fields:**
- `delivery_type` - "daily", "training", "custom", `"blockgraph_syndicated"`, `"blockgraph_custom"`
...
- `audience_bitmap_path` - Source for audience evaluation: `"tagav"` (default, all vendors) or `"person_jar"` (BlockGraph)
- `person_jar_path` - Required for BlockGraph delivery types; S3 path to personJar bitmap

### BlockGraph (FreeWheel) Delivery

BlockGraph is **person-keyed (RID)**, not cookie-keyed (RCID). Key differences from other vendors:

- **Two delivery types:** `blockgraph_syndicated` (standard audience delivery) and `blockgraph_custom` (custom audience key list)
- **Multi-column stitch (`stitch_columns`):** BG uses normalized address columns (`norm_address_line`, `norm_city`, `norm_state`, `norm_zip`, `zip_plus4`) against `person_identity_graph_beta` table — unlike other vendors that use a single `stitch_column`
- **`audience_bitmap_path = "person_jar"`:** BEM evaluates against personJar rather than SuperTagAv for BG delivery
- **`person_jar_path` required:** Must be present in the event for BG runs; the config Lambda fails fast if missing
- **~872 audiences** in the BlockGraph ADS group (as of 2026-06 dev verification)

The `stitch_columns` (plural) config key is generic — it can be used for any future vendor needing multi-column stitch, alongside the existing singular `stitch_column`. Downstream multi-column stitch consumption (T05 / CDP-118914) is handled separately from the config Lambda.

### batch-stitch Rate Limiting

The `batch-stitch-pipeline` step function uses `MaxConcurrency=2` on its `AddJobFlowSteps` Map state (CDP-118972). This prevents EMR API throttling when running many parallel stitch jobs. Do not increase this without verifying EMR API rate limits.

### Formatter Delta Path and Custom Delivery

The formatter Lambda (`batch-delivery-formatter-pipeline`) uses `source_prefix` to locate stitched output. After CDP-118937, the `source_prefix` for custom delivery is aligned to match the new formatter path layout. When adding new vendors, ensure `source_prefix` in `vendor-config.ini` matches the formatter's expected S3 path structure.
